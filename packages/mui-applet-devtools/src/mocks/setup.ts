...
import { registerHostMockOverrides } from "..";
import { mockOverrides } from ".";
...
export function initializeMockOverrides(): void {
  if (typeof window !== "undefined") {
    ...
    console.log(
      "🎭 Host mock overrides registered:",
      mockOverrides.length,
      "handlers",
    );
  }
}
...
export async function autoInitializeMockOverrides(): Promise<void> {
  if (typeof window !== "undefined") {
    try {
      ...
      console.warn(
        "Could not auto-detect mock mode, use initializeMockOverrides() manually",
      );
    }
  }
}
