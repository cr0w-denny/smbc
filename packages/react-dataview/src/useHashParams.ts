...
>(
  ...
  filters: TFilters;
  setFilters: (updates: Partial<TFilters> | ((prev: TFilters) => TFilters)) => void;
  pagination: TPagination;
  setPagination: (updates: Partial<TPagination> | ((prev: TPagination) => TPagination)) => void;
} {
  ...
  const updateHashParams = useCallback((newFilters: TFilters, newPagination: TPagination) => {
    ...
    try {
      ...
      if (hash) {
        ...
        if (queryString) {
          ...
          for (const [key, value] of existingParams.entries()) {
            ...
            if (!(cleanKey in defaultFilters) && !(cleanKey in defaultPagination)) {
      ...
      console.warn('Failed to update hash params:', error);
    }
  }, [enabled, getCurrentPath, namespace, defaultFilters, defaultPagination, addNamespace]);

  // Debounced hash update
  const debouncedUpdateHash = useCallback((newFilters: TFilters, newPagination: TPagination) => {
  ...
  // Filter setter
  const setFilters = useCallback((updates: Partial<TFilters> | ((prev: TFilters) => TFilters)) => {
    setFiltersState(prev => {
      const newFilters = typeof updates === 'function' ? updates(prev) : { ...prev, ...updates };
  ...
  // Pagination setter
  const setPagination = useCallback((updates: Partial<TPagination> | ((prev: TPagination) => TPagination)) => {
    setPaginationState(prev => {
      const newPagination = typeof updates === 'function' ? updates(prev) : { ...prev, ...updates };
