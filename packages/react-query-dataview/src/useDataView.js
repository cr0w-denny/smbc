...
export function useDataView(config, options = {}) {
    const { api, schema, columns, renderer, filters: filterSpec, actions: actionConfig = {}, pagination: paginationConfig = { enabled: true, defaultPageSize: 10 }, forms, options: rendererOptions = {}, activity: activityConfig, } = config;
    ...
    const emitActivity = useCallback((type, item) => {
    ...
    const hashParamsEnabled = options.hashParams !== false &&
    ...
    const responseRow = api.responseRow ||
        ((response) => (Array.isArray(response) ? response : []));
    const responseRowCount = api.responseRowCount ||
        ((response) => response?.total || response?.count || responseRow(response).length);
    const optimisticResponse = api.optimisticResponse ||
        ((_originalResponse, newRows) => newRows);
    ...
    const updatePendingStates = React.useCallback((updater) => {
        ...
        if (transactionManager) {
            ...
            const handleTransactionCancelled = () => {
                ...
                if (transactionSnapshot) {
                    ...
                    queryClient.setQueryData(currentQueryKeyRef.current, (old) => {
                        if (!old)
                            return old;
            ...
            const handleTransactionComplete = (results) => {
            ...
            const handleOperationRemoved = (operationId, removedOperation) => {
                ...
                queryClient.setQueryData(currentQueryKeyRef.current, (old) => {
                    ...
                    if (removedOperation && removedOperation.type === "create") {
                        ...
                        const entityId = removedOperation.entityId;
                        updatedRows = currentRows.filter((item) => item[schema.primaryKey] !== entityId);
                    }
                    else {
    ...
    const pendingStatesRef = React.useRef(new Map());
    ...
    const { data: queryData, isLoading, error, } = useQuery({
        ...
        queryFn: async () => {
            const response = await api.client.GET(api.endpoint, queryParams);
            if (response.error) {
    ...
    console.log('UseDataView: Data being returned to table', {
        ...
        dataItemIds: data.map((item) => item[schema.primaryKey])
    });
    ...
    const createMutation = useMutation({
        mutationFn: async (variables) => {
            const response = await api.client.POST(api.endpoint, variables);
            if (response.error) {
        ...
        onSuccess: (newItem) => {
            ...
            queryClient.setQueryData(currentQueryKey, (old) => {
                ...
                const updatedRows = currentRows.map((item) => {
                    ...
                    if (typeof item[schema.primaryKey] === "string" &&
    ...
    const updateMutation = useMutation({
        mutationFn: async (variables) => {
            const response = await api.client.PATCH(`${api.endpoint}/{id}`, variables);
            if (response.error) {
        ...
        onMutate: async (variables) => {
            ...
            queryClient.setQueryData(currentQueryKey, (old) => {
                ...
                const updatedRows = currentRows.map((item) => item[primaryKey] === itemId ? { ...item, ...variables.body } : item);
                return optimisticResponse(old, updatedRows);
            });
            return { previousData };
        },
    ...
    const deleteMutation = useMutation({
        mutationFn: async (variables) => {
            const response = await api.client.DELETE(`${api.endpoint}/{id}`, variables);
            if (response.error) {
        ...
        onMutate: async (variables) => {
            ...
            queryClient.setQueryData(currentQueryKey, (old) => {
                ...
                const filteredRows = currentRows.filter((item) => item[primaryKey] !== itemId);
    ...
    const getPendingData = useCallback((entityId) => {
        if (!transactionManager || !transactionManager.hasOperations()) {
            return null;
        }
    ...
    const addTransactionOperation = useCallback(async (type, entity, mutation, trigger = "user-edit", changedFields) => {
        ...
        const originalItem = previousData
            ? responseRow(previousData).find((item) => item[schema.primaryKey] === entity[schema.primaryKey])
            : undefined;
    ...
    const getMergedItem = useCallback((item) => {
        if (!transactionManager || !transactionManager.hasOperations()) {
            return item;
        }
        const primaryKey = schema.primaryKey;
    ...
    const handleDelete = useCallback(async (item) => {
        ...
        if (transactionManager) {
            const primaryKey = schema.primaryKey;
            try {
                await addTransactionOperation("delete", item, () => deleteMutation.mutateAsync({
    ...
    const handleCreateSubmit = useCallback(async (data) => {
        ...
        try {
            await addTransactionOperation("create", tempEntity, () => createMutation.mutateAsync({ body: data }), "user-edit");
    ...
    const handleEditSubmit = useCallback(async (data) => {
        ...
        try {
            await addTransactionOperation("update", updatedEntity, () => updateMutation.mutateAsync({
    ...
    const handleDeleteConfirm = useCallback(async () => {
        ...
        const primaryKey = schema.primaryKey;
        try {
            await addTransactionOperation("delete", deletingItem, () => deleteMutation.mutateAsync({
    ...
    const selectedItems = useMemo(() => {
        const primaryKey = schema.primaryKey;
        return data.filter((item) => selectedIds.includes(item[primaryKey]));
    ...
    const FilterComponent = useMemo(() => {
        if (!filterSpec)
            return () => null;
        return () => React.createElement(renderer.FilterComponent, {
    ...
    const CreateFormComponent = useMemo(() => {
        ...
        if (!createForm)
            return () => null;
        const entityType = activityConfig?.entityType || "Item";
        return () => React.createElement(renderer.FormComponent, {
            mode: "create",
    ...
    const EditFormComponent = useMemo(() => {
        ...
        if (!editForm)
            return () => null;
        const entityType = activityConfig?.entityType || "Item";
        return ({ item }) => React.createElement(renderer.FormComponent, {
            mode: "edit",
            fields: renderer.mapFormFields
    ...
    const PaginationComponent = useMemo(() => {
        if (!paginationConfig.enabled)
            return () => null;
        return () => React.createElement(renderer.PaginationComponent, {
            page: pagination.page,
            pageSize: pagination.pageSize,
            total,
            onPageChange: (page) => setPagination({ page }),
            onPageSizeChange: (pageSize) => setPagination({ pageSize, page: 0 }),
