...
export class SimpleTransactionManager {
    ...
    addOperation(operation, providedId) {
        if (!this.currentTransaction) {
            this.begin(); // Auto-start transaction if needed
        }
        // Check for conflicting operations on the same entity
        const existingOpIndex = this.currentTransaction.operations.findIndex((op) => op.entityId === operation.entityId);
        const id = providedId ||
    ...
    async commit(force = false) {
        ...
        try {
            ...
            this.emit("onTransactionError", error, transaction);
            throw error;
        }
    }
    cancel() {
    ...
    getSummary() {
        ...
        const entities = [
            ...new Set(operations.map((op) => typeof op.entity === "object" &&
                op.entity &&
    ...
    async rollbackOperations(successfulResults) {
    ...
    async rollbackSingleOperation(_operation) {
