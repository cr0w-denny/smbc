...
// Generated at: 2025-07-03T00:09:39.423Z
...
let createuserrequestDataStore = new Map();
...
function initializeCreateUserRequestDataStore() {
    if (createuserrequestDataInitialized)
        return;
    const totalItems = faker.number.int(mockConfig.dataSetSize);
    const items = Array.from({ length: totalItems }, () => generateCreateUserRequest({}));
...
function getAllCreateUserRequests() {
...
let errorresponseDataStore = new Map();
...
function initializeErrorResponseDataStore() {
    if (errorresponseDataInitialized)
        return;
    const totalItems = faker.number.int(mockConfig.dataSetSize);
    const items = Array.from({ length: totalItems }, () => generateErrorResponse({}));
    items.forEach((item, index) => {
...
let updateuserrequestDataStore = new Map();
...
function initializeUpdateUserRequestDataStore() {
    if (updateuserrequestDataInitialized)
        return;
    const totalItems = faker.number.int(mockConfig.dataSetSize);
    const items = Array.from({ length: totalItems }, () => generateUpdateUserRequest({}));
    items.forEach((item, index) => {
...
// @ts-ignore - May not be used by all operations
function getAllUpdateUserRequests() {
...
function initializeUserDataStore() {
    if (userDataInitialized)
        return;
    const totalItems = faker.number.int(mockConfig.dataSetSize);
    const items = Array.from({ length: totalItems }, (_, i) => generateUser({ id: String(i + 1) }));
    items.forEach((item) => {
...
function transformUserToSchema(item, targetSchema) {
    switch (targetSchema) {
        case 'UserSummary': {
...
let userdetailedDataStore = new Map();
...
function initializeUserDetailedDataStore() {
    if (userdetailedDataInitialized)
        return;
    const totalItems = faker.number.int(mockConfig.dataSetSize);
    const items = Array.from({ length: totalItems }, (_, i) => generateUserDetailed({ id: String(i + 1) }));
    items.forEach((item) => {
...
function initializeUserListDataStore() {
    if (userlistDataInitialized)
        return;
    const totalItems = faker.number.int(mockConfig.dataSetSize);
    const items = Array.from({ length: totalItems }, () => generateUserList({}));
    items.forEach((item, index) => {
...
let usersummaryDataStore = new Map();
...
function initializeUserSummaryDataStore() {
    if (usersummaryDataInitialized)
        return;
    const totalItems = faker.number.int(mockConfig.dataSetSize);
    const items = Array.from({ length: totalItems }, (_, i) => generateUserSummary({ id: String(i + 1) }));
    items.forEach((item) => {
...
export const handlers = [
    ...
    http.get(`${mockConfig.baseUrl}/users`, async ({ request }) => {
        ...
        if (faker.number.float() < mockConfig.errorRate) {
            return HttpResponse.json({ error: 'Not found', message: 'User not found' }, { status: 404 });
        }
        ...
        // Apply search filter
        if (search) {
            filteredItems = filteredItems.filter((item) => ['email', 'firstName', 'lastName']
        ...
        const schemaMapping = { "summary": "UserSummary", "detailed": "UserDetailed" };
    ...
    http.post(`${mockConfig.baseUrl}/users`, async ({ request }) => {
        ...
        // Error simulation based on configuration
        if (faker.number.float() < mockConfig.errorRate) {
            return HttpResponse.json({ error: 'Validation failed', message: 'Invalid user data' }, { status: 400 });
        }
