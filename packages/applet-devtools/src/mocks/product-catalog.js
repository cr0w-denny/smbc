...
// Generated at: 2025-07-03T00:09:31.344Z
...
let createproductrequestDataStore = new Map();
...
function initializeCreateProductRequestDataStore() {
    if (createproductrequestDataInitialized)
        return;
    const totalItems = faker.number.int(mockConfig.dataSetSize);
    const items = Array.from({ length: totalItems }, () => generateCreateProductRequest({}));
...
function getAllCreateProductRequests() {
...
function initializeProductDataStore() {
    if (productDataInitialized)
        return;
    const totalItems = faker.number.int(mockConfig.dataSetSize);
    const items = Array.from({ length: totalItems }, (_, i) => generateProduct({ id: String(i + 1) }));
    items.forEach((item) => {
...
let productlistDataStore = new Map();
...
function initializeProductListDataStore() {
    if (productlistDataInitialized)
        return;
    const totalItems = faker.number.int(mockConfig.dataSetSize);
    const items = Array.from({ length: totalItems }, () => generateProductList({}));
    items.forEach((item, index) => {
...
let updateproductrequestDataStore = new Map();
...
function initializeUpdateProductRequestDataStore() {
    if (updateproductrequestDataInitialized)
        return;
    const totalItems = faker.number.int(mockConfig.dataSetSize);
    const items = Array.from({ length: totalItems }, () => generateUpdateProductRequest({}));
    items.forEach((item, index) => {
...
// @ts-ignore - May not be used by all operations
function getAllUpdateProductRequests() {
...
export const handlers = [
    ...
    http.get(`${mockConfig.baseUrl}/products`, async ({ request }) => {
        ...
        // Apply search filter
        if (search) {
            filteredItems = filteredItems.filter((item) => JSON.stringify(item).toLowerCase().includes(search.toLowerCase()));
        }
    ...
    http.post(`${mockConfig.baseUrl}/products`, async ({ request }) => {
        ...
        // Error simulation based on configuration
        if (faker.number.float() < mockConfig.errorRate) {
            return HttpResponse.json({ error: 'Validation failed', message: 'Invalid product data' }, { status: 400 });
        }
