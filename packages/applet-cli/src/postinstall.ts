...
if (isCI || skipSetup) {
  console.log(
    "⏭️  Skipping interactive setup (CI environment or SKIP_APPLET_SETUP=true)",
  );
  process.exit(0);
}
...
async function ensureTsxInstalled(): Promise<void> {
  try {
    ...
    const packageJson = JSON.parse(readFileSync(packageJsonPath, "utf8"));
    const hasTsx =
      packageJson.devDependencies?.tsx || packageJson.dependencies?.tsx;
...
async function setupApplets(): Promise<void> {
  try {
    ...
    if (framework === "unknown") {
      console.log(
        "⚠️  Could not detect framework. Please ensure you have @mui/material or similar installed.",
      );
      console.log(
        "   You can still create a basic config and add applets manually later.",
      );
      console.log("");
    } else {
    ...
    const response = await prompts({
      ...
      choices: availableApplets.map((pkg) => ({
        title: pkg.replace("@smbc/", "").replace("-mui", ""),
        value: pkg,
      })),
    ...
    // Generate config with selected packages
    ...
    if (selectedApplets.length > 0) {
      ...
      const packageSpecs = selectedApplets;

      const installProcess = spawn(
        packageManager,
        [installCmd, ...packageSpecs],
        {
          stdio: "inherit",
          cwd: process.cwd(),
          shell: true,
      ...
      installProcess.on("close", (code) => {
        if (code === 0) {
          ...
          console.log("");
          console.log(
            "⚠️  Applet installation failed. You can install them manually:",
          );
          selectedApplets.forEach((pkg: string) => {
            console.log(`   ${packageManager} ${installCmd} ${pkg}`);
          });
        }
      });
    } else {
...
function generateConfig(framework: string, selectedPackages: string[]): string {
  // Generate imports and basic metadata from package names
  const imports = selectedPackages
    .map((pkg) => {
      // Convert @smbc/hello-mui to HelloApplet
      const appletName = pkg.replace("@smbc/", "").replace("-mui", "");
      const exportName =
        appletName.charAt(0).toUpperCase() +
        appletName.slice(1).replace(/-./g, (match) => match[1].toUpperCase()) +
        "Applet";
      return `import ${exportName} from '${pkg}';`;
    })
    .join("\n");

  const appletConfigs = selectedPackages
    .map((pkg) => {
      const appletName = pkg.replace("@smbc/", "").replace("-mui", "");
      const exportName =
        appletName.charAt(0).toUpperCase() +
      ...
      const displayName =
        appletName.charAt(0).toUpperCase() +
        appletName.slice(1).replace(/-/g, " ");

      return `  mountApplet(${exportName}, {
    id: "${appletName}",
    label: "${displayName}",
    path: "/${appletName}",
    icon: HomeIcon, // Update with appropriate icon
    permissions: [],
  ...
  const permissionConfigs = selectedPackages
    .map((pkg) => {
      const appletName = pkg.replace("@smbc/", "").replace("-mui", "");
      const exportName =
        appletName.charAt(0).toUpperCase() +
      ...
      return `  "${appletName}": minRole(${exportName}, {
