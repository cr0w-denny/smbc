 ...
 */
export function extractQueryParameters(operation) {
    const parameters = [];
 ...
 */
export function parameterToFieldConfig(param) {
 ...
 */
export function schemaToFieldConfig(name, schema, options = {}) {
    const config = {
        name,
        ...
        placeholder: options.description || `Enter ${generateLabel(name).toLowerCase()}`,
    };
    ...
    switch (schema.type) {
        case 'string':
            if (schema.enum) {
                config.type = 'select';
                config.options = schema.enum.map((value) => ({
                    value,
    ...
    if (name.toLowerCase().includes('sort')) {
        if (name.toLowerCase().includes('order') || name.toLowerCase().includes('direction')) {
            config.type = 'select';
            config.options = [
 ...
 */
export function validateFilterValues(values, fields) {
    const errors = {};
    fields.forEach((field) => {
 ...
 */
export function cleanFilterValues(values) {
    const cleaned = {};
