...
async function delay() {
  const delayMs = faker.number.int({
    min: mockConfig.delay.min,
    max: mockConfig.delay.max,
  });
...
function generateEvent(overrides = {}) {
  return {
    event_ref_id: faker.string.alphanumeric({
      length: 8,
      casing: "upper",
    }),
    ...
    event_date: format(
      faker.date.between({ from: "-30d", to: "+30d" }),
      "yyyy-MM-dd",
    ),
    event_res_date: format(
      faker.date.between({ from: "+1d", to: "+60d" }),
      "yyyy-MM-dd",
    ),
    event_category: faker.helpers.arrayElement(["Mandatory", "Discretionary"]),
    workflow_status: faker.helpers.arrayElement([
      "Subscribed",
      "NotSubscribed",
      "Review",
      "Approval",
    ...
    lifecycle_status: faker.helpers.arrayElement([
      "on-course",
      "almost-due",
      "past-due",
      "needs-attention",
    ...
    trigger_shortname: faker.helpers.arrayElement([
      "RTNG_DOWN_1NOTCH",
      "STCK_PRC_DOWN_L1",
    ...
    trigger_values: faker.helpers.arrayElement([
...
function initializeEventDataStore() {
  ...
  const totalItems = faker.number.int({
    min: mockConfig.dataSetSize.min,
    max: mockConfig.dataSetSize.max,
  });
...
export const handlers = [
  http.get(`${mockConfig.baseUrl}/events`, async ({ request }) => {
    ...
    // LOG ALL HEADERS TO VERIFY X-IMPERSONATE PRESENCE
    const headers = Object.fromEntries(request.headers.entries());
    const impersonateHeader = request.headers.get('X-Impersonate');

    console.log('🔍 MSW Handler - /events called at', new Date().toISOString());
    console.log('🔍 MSW Handler - Request URL:', request.url);
    console.log('🔍 MSW Handler - X-Impersonate header:', impersonateHeader);
    console.log('🔍 MSW Handler - All headers:', headers);

    if (impersonateHeader) {
      console.log('✅ MSW Handler - X-Impersonate header FOUND:', impersonateHeader);
    } else {
      console.log('❌ MSW Handler - X-Impersonate header NOT FOUND');
    }

    // Return 404 for testing ag-grid error handling
    ...
    if (status !== null && status !== "") {
      filteredItems = filteredItems.filter(
        (item: any) =>
          item.lifecycle_status?.toString().toLowerCase() ===
          status.toLowerCase(),
      );
    }
    if (types !== null && types !== "") {
